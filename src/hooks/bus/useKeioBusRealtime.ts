...
// 取得したい車両位置情報の型定義
export interface KeioBusVehicle {
  id: string;              // 車両の一意のID
...
export const useKeioBusRealtime = (isActive: boolean = true) => {
  const [vehicles, setVehicles] = useState<KeioBusVehicle[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchKeioBusData = async () => {
      ...
      try {
        ...
        const proxyUrl = `/api-keio/api/v4/gtfs/realtime/odpt_KeioBus_AllLines_vehicle?acl:consumerKey=${API_KEY}`;
        ...
        const parsedVehicles: KeioBusVehicle[] = [];
        ...
        console.log(`[京王バス] ${parsedVehicles.length} 台のリアルタイム位置情報を取得しました。`);
        console.log("サンプルデータ:", parsedVehicles.slice(0, 20)); // 最初の20台だけ表示
        setVehicles(parsedVehicles);

      } catch (err: any) {
        console.error("京王バスのリアルタイムデータデコードに失敗しました:", err);
    ...
    // 30秒ごとに自動リフレッシュ
    const interval = setInterval(fetchKeioBusData, 30000);
