...
const OPERATOR_MAP: Record<string, string> = {
  "odpt.Operator:YokohamaMunicipal": "横浜市営バス",
...
export interface ActiveBus {
  ...
  nextNextBusstopName: string | null; // 次の次のバス停名
  arrivalEstimateTime: string | null; // 計算した到着予定時刻 (例: "14:35")
  delayMin: number;
...
export const useActiveKanagawaBuses = (busstops: any[]) => {
  ...
  useEffect(() => {
    ...
    const fetchAllBusLocations = async () => {
      ...
      try {
        ...
        // ----------------------------------------------------
        const odptUrl = `https://api.odpt.org/api/v4/odpt:Bus?odpt:operator=odpt.Operator:YokohamaMunicipal&acl:consumerKey=${API_KEY}`;
        const kawasakiCityProxyUrl = `https://api.odpt.org/api/v4/gtfs/realtime/odpt_TransportationBureau_CityOfKawasaki_AllLines_vehicle?acl:consumerKey=${API_KEY}`;
        ...
        const [resOdpt, resKawa] = await Promise.all([
          fetch(odptUrl).catch(e => { console.error("ODPT API障害:", e); return null; }),
          fetch(kawasakiCityProxyUrl).catch(e => { console.error("京王GTFS-R障害:", e); return null; }),
        ...
        // ====================================================
        // 🚌 Bグループ: 川崎市営バス（GTFS-Realtime バイナリ形式）の処理
        // ====================================================
        if (resKawa && resKawa.ok) {
          const arrayBuffer = await resKawa.arrayBuffer();
          ...
          for (const entity of feed.entity) {
            ...
            // 川崎市営バスの stopId（向かっている、または停車中のバス停ID）を取得
            const currentStopId = `kawasakicity:BusstopPole:${v.stopId}` || null;
            if (!currentStopId) continue;

            // 画面内のバス停リストの中に、川崎市営バスが位置するバス停が含まれる場合のみ採用
            if (screenStopIds.has(currentStopId)) {
              const matchedStop = busstops.find(s => s["owl:sameAs"] === currentStopId)!;
              // console.log(`川崎市営バスが位置するバス停を発見: ${matchedStop["dc:title"]} (${currentStopId})`);

              // 川崎市営バスの系統ID・便IDを取得
              const gtfsRouteId = v.trip?.tripId || null;
              // console.log(`川崎市営バスのGTFS v.trip: ${v.trip ? JSON.stringify(v.trip) : "情報なし"}`);

              // ODPT互換のbusroutePattern形式に擬態させる
              const simulatedPatternId = gtfsRouteId ? `kawasakicity:BusroutePattern:${gtfsRouteId}` : null;
              // console.log(`川崎市営バスのシミュレートされたbusroutePattern: ${simulatedPatternId}`);

              let kawasakiNextPoleId: string | null = null;
              let kawasakiNextName: string | null = null;
              let kawasakiNextNextName: string | null = null;
              let kawasakiEstimateTime: string | null = null;
              ...
              if (simulatedPatternId) {
                const { data: matchedObjects } = await supabase
                  ...
                  .ilike("timetable_owl_sameas", `kawasakicity:Timetable:${gtfsRouteId}%`);
                ...
                if (matchedObjects && matchedObjects.length > 0) {
                  ...
                  if (correctStopData && correctStopData.index_order !== undefined) {
                    ...
                    // --------------------------------------------------------
                    // 💡 川崎市営バス用：現在の時間と予定時刻から遅延（分）を算出する
                    // --------------------------------------------------------
                    let scheduledTotalMinutes = 0;
                    if (correctStopData.departure_time) {
                    ...
                    if (nxtList && nxtList.length > 0 && nxtList[0].busstop_pole_owl_sameas) {
                      ...
                      kawasakiNextPoleId = nxtList[0].busstop_pole_owl_sameas;
                      ...
                      if (predictedTime) {
                        ...
                        // "05:09" のように2桁にパディングして文字列化
                        kawasakiEstimateTime = `${String(resHours).padStart(2, '0')}:${String(resMinutes).padStart(2, '0')}`;
                      }
                      else {
                        kawasakiEstimateTime = null; // 予測時間が計算できない場合は null にする
                      }
                      console.log(`川崎市営バスの到着時間: ${correctStopData.departure_time}, 予測到着時刻: ${kawasakiEstimateTime}, 遅延時間: ${calculatedDelayMin} 分`);
                      ...
                      const { data: nxtPoleData } = await supabase
                          ...
                          .maybeSingle();
                        if (nxtPoleData) kawasakiNextName = nxtPoleData.title;
                      ...
                      if (nxtNextList && nxtNextList.length > 0 && nxtNextList[0].busstop_pole_owl_sameas) {
                        const { data: nxtNextPoleData } = await supabase
                          ...
                          .maybeSingle();
                        if (nxtNextPoleData) kawasakiNextNextName = nxtNextPoleData.title;
                      }
                    }
                  }
                }
              ...
              integratedBuses.push({
                id: `kawasakiBus:${v.vehicle?.id || Math.random().toString()}`,
                operatorName: "川崎市営バス",
                busNumber: v.vehicle?.id || "不明",
                fromStationName: matchedStop["dc:title"], // 現在関わりのある停留所名
                toBusstopPoleId: kawasakiNextPoleId,
                nextBusstopName: kawasakiNextName || null,
                nextNextBusstopName: kawasakiNextNextName || null,
                arrivalEstimateTime: kawasakiEstimateTime,
                delayMin: calculatedDelayMin,
                // 💡 川崎市営は、バス停固定座標ではなく「GPSが返してきた本物の現在位置」を優先してマーカー配置可能！
        ...
        // ====================================================
        // 🚌 Cグループ: 伊豆箱根バス（GTFS-Realtime バイナリ形式）の処理
        // ====================================================
