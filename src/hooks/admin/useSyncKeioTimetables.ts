  ...
  const syncTimetables = async (): Promise<{ success: boolean; count?: number; error?: string }> => {
    ...
    try {
      ...
      for (const st of stopTimes) {
        ...
        // 京王バス用の識別IDを組み立てる (事業者:バス停ID:運行パターン:カレンダー区分)
        const timetableId = `keio:Timetable:${st.trip_id}:${tripInfo.serviceId}`;

        let type: "weekday" | "saturday" | "holiday" = "weekday";

        if (tripInfo.serviceId.includes("土曜")) {
            type = "saturday";
          } else if (
            tripInfo.serviceId.includes("休日") || 
            tripInfo.serviceId.includes("日祝") || 
            tripInfo.serviceId.includes("日曜") || 
            tripInfo.serviceId.includes("祝日")
          ) {
            type = "holiday";
          } else {
            type = "weekday";
        ...
        if (!timetablesMap.has(timetableId)) {
          timetablesMap.set(timetableId, {
            ...
            busroute: `keio:Busroute:${tripInfo.routeId}`,
            busroute_pattern: `keio:BusroutePattern:${st.trip_id}`,
            calendar: type,
        ...
        timetableObjects.push({
          ...
          busstop_pole_owl_sameas: `keio:BusstopPole:${st.stop_id}`,
