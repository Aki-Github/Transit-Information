...
const LINE_OPTIONS: LineOption[] = [
  { line_cd: '22004', line_name: '西武豊島園線' },
  { line_cd: '22001', line_name: '西武池袋線' },
  { line_cd: '22007', line_name: '西武新宿線' },
  { line_cd: '22008', line_name: '西武拝島線' },
  { line_cd: '22005', line_name: '西武狭山線' },
  { line_cd: '22006', line_name: '西武山口線' },
  { line_cd: '22009', line_name: '西武西武園線' },
  { line_cd: '22010', line_name: '西武国分寺線' },
  { line_cd: '22011', line_name: '西武多摩湖線' },
  { line_cd: '22012', line_name: '西武多摩川線' },
  { line_cd: '21001', line_name: '東武東上線' },
  { line_cd: '11311', line_name: 'JR中央線' },
  { line_cd: '11305', line_name: 'JR武蔵野線' },
  { line_cd: '11332', line_name: 'JR京浜東北線' },
  { line_cd: '11322', line_name: 'JR川越線' },
  { line_cd: '99301', line_name: '都営大江戸線' },
  { line_cd: '11317', line_name: 'JR八高線' },
  { line_cd: '11315', line_name: 'JR青梅線' },
];

const operator: string = 'odpt.Operator:SeibuBus';
...
// 🚀 コンポーネント本体
// ==========================================
export const TerminalDepartureBoardSeibu: FC = memo(() => {
  // フックから座標更新関数（searchByCoordinates）も追加で受け取る
  const { terminalBoards, loading, errorMessage, searchByCoordinates } : UseBusPoleTimeTableReturn = useBusPoleTimeTable([35.737893, 139.654368], operator);

  // 選択中の路線コード (初期値は豊島園線)
  const [selectedLineCd, setSelectedLineCd] = useState<string>('22004');
  ...
  // 表示中の駅名を取得
  const currentStationName = `${stations[parseInt(selectedStationIndex, 10)]?.station_name}駅周辺` || "練馬駅周辺";
    ...
    <Box maxW="1000px" mx="auto" p="5">
      ...
      <Box {...departureStyles.boardContainer}>
        ...
        <Flex {...departureStyles.boardHeader}>
          <Heading size="md" letterSpacing="2px" color="#0bbfe3">
            🔵 {currentStationName} 西武バス 発車案内板
          </Heading>
        ...
        <Grid templateColumns={{ base: "1fr", md: "1fr 1fr" }} gap="5">
          {safeTerminalBoards.length > 0 ? (
            safeTerminalBoards.map((platform, idx) => (
              <Box key={idx} {...departureStyles.platformCardSeibu}>
                
                {/* のりばタイトル */}
                <Flex {...departureStyles.platformHeaderSeibu}>
                  <Text {...departureStyles.platformTitle}>
                    🚏 {platform.platformName}
                  </Text>
                  <Badge {...departureStyles.platformBadgeSeibu}>
                    先発・次発
                  </Badge>
                </Flex>
