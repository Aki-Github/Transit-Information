...
const LINE_OPTIONS: LineOption[] = [
  { line_cd: '11307', line_name: 'JR根岸線' },
  { line_cd: '11306', line_name: 'JR横浜線' },
  { line_cd: '11308', line_name: 'JR横須賀線' },
  { line_cd: '11332', line_name: 'JR京浜東北線' },
  { line_cd: '27001', line_name: '京急本線' },
  { line_cd: '29001', line_name: '相鉄本線' },
  { line_cd: '99310', line_name: 'みなとみらい線' },
  { line_cd: '99316', line_name: '横浜市営地下鉄ブルーライン' },
  { line_cd: '99343', line_name: '横浜市営地下鉄グリーンライン' },
];

const operator: string = 'odpt.Operator:YokohamaMunicipal';
...
// 🚀 コンポーネント本体
// ==========================================
export const TerminalDepartureBoardYokohama: FC = memo(() => {
  // フックから座標更新関数（searchByCoordinates）も追加で受け取る
  const { terminalBoards, loading, errorMessage, searchByCoordinates } : UseBusPoleTimeTableReturn = useBusPoleTimeTable([35.466195, 139.622704], operator);

  // 選択中の路線コード (初期値は豊島園線)
  const [selectedLineCd, setSelectedLineCd] = useState<string>('11307');
  ...
  // 表示中の駅名を取得
  const currentStationName = `${stations[parseInt(selectedStationIndex, 10)]?.station_name}駅周辺` || "横浜駅周辺";
    ...
    <Box maxW="1000px" mx="auto" p="5">
      ...
      <Box {...departureStyles.boardContainer}>
        ...
        <Flex {...departureStyles.boardHeader}>
          <Heading size="md" letterSpacing="2px" color="#007FFF">
            🔵 {currentStationName} 横浜市営バス 発車案内板
          </Heading>
        ...
        <Grid templateColumns={{ base: "1fr", md: "1fr 1fr" }} gap="5">
          {safeTerminalBoards.length > 0 ? (
            safeTerminalBoards.map((platform, idx) => (
              <Box key={idx} {...departureStyles.platformCardYokohama}>
                
                {/* のりばタイトル */}
                <Flex {...departureStyles.platformHeaderYokohama}>
                  <Text {...departureStyles.platformTitle}>
                    🚏 {platform.platformName}
                  </Text>
                  <Badge {...departureStyles.platformBadgeYokohama}>
                    先発・次発
                  </Badge>
                </Flex>
