...
const isUpTrain = (train: ToeiTrainData): boolean => {
  ...
  const downKeywords = [
    "Motoyawata", "Iwamotocho", "Ojima", "Mizue",
  ];
...
export const Shinjuku: FC = memo(() => {
  ...
  const styles = recipe();
  const lineId = "Shinjuku"; // 追加: 使用する路線IDを定義
  ...
  const fetchShinjukuTrains = async () => {
    try {
      const API_KEY = process.env.REACT_APP_ODPT_KEY;
      const url = `https://api.odpt.org/api/v4/odpt:Train?odpt:railway=odpt.Railway:Toei.Shinjuku&acl:consumerKey=${API_KEY}`;
      
      const res = await fetch(url);
      if (!res.ok) throw new Error(`HTTPエラー: ${res.status}`);
      const data: ToeiTrainData[] = await res.json();
      console.log("新宿線データ:", data);
      
      // データが配列であることの確認ガード
      setTrains(Array.isArray(data) ? data : []);
      setError(null);
    } catch (e: any) {
      console.error("新宿線データ取得失敗", e);
  ...
  useEffect(() => {
    fetchShinjukuTrains();
    const interval = setInterval(fetchShinjukuTrains, 20000);
  ...
  if (loading || stationsLoading || destinationLoading) {
    return (
      <Center h="100vh" bg="gray.950">
        ...
        <Text ml="4" color="white">新宿線ナビ 読み込み中...</Text>
      </Center>
    );
  }
